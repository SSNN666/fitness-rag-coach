...
## 🗣️ 2026-09-11 修「依据原文步骤」噪声 + 页脚措辞撞车

**发现**：用户拿一条真实回答问「这一堆（依据原文步骤）是什么」——`给我一个一周训练计划`
的回答里，这句空话出现了 **15 次**，逐行贴在每个动作后面。**代码与知识库里都没有这个词**
（grep 确认），是模型生成的。

**根因**：分层 Prompt 写的是「每个动作、组数、频率**必须在「参考知识」中有原文依据**」
（计划层）/「每条建议必须在「参考知识」中有原文依据」（伤病层）。模型把它读成了
**标注义务**——要在每行后面盖章证明自己没编；但它拿不到可引用的粒度
（知识库是整条 CSV 行，不是「第 3 步」），只能退化成一句没有信息量的占位语。

**为什么是真缺陷**：系统**本来就有独立的引用机制**（`_build_citations` → `citations`
字段，UI 可展开「引用来源」）——正文再标一遍是**纯冗余**；更糟的是它**掩盖了一个
本该暴露的问题**：提示词要求「引用具体动作名/禁忌/肌群」，模型做不到，用占位符糊过去了。

| # | 项 | 文件 | 说明 |
|---|---|---|---|
| 1 | 要求从「标注义务」改为「内容约束」 | pipeline.py | 计划层/伤病层均改为「**只能取自「参考知识」**」+ 明文**禁止正文标注出处**。具体化要求保留（原意是「别泛泛而谈」，不是「逐条盖章」） |
| 2 | 缓存版本 bump | config.py | `FACT_CACHE_VERSION` 2 → 3。**不 bump 的话带标注的旧答案会继续从缓存吐出**，看起来像没修好（这正是该机制存在的意义） |
| 3 | 页脚措辞撞车 | pipeline.py / app.py | 「本次使用**快速模式**回答」与 `simple/plan/injury` **分层**撞名——计划层用户会以为被降级到简单层，而计划层实际走主模型 + 重排 + 校验的完整链路。改为「本次未开启「深度思考」」（`deep_thinking` 才是这里说的东西） |
| 4 | 回归守卫 | tests/test_pipeline.py | +5 项：两条 Prompt 均不得含「必须有原文依据」这类诱导措辞、必须含禁止标注约束、具体化要求不丢；`_mode_hint` 不得出现「快速模式」且 simple 层/已开深度思考时为空 |

**过程中守卫测试抓到我自己的错**：我先把「原文『…有原文依据』被模型执行成逐行贴标签」
这段解释**写进了 Prompt 字符串里**当注脚——等于**把旧措辞又喂回给模型**，可能重新诱发
标注行为，且当场触发了我自己写的断言。解释已移到 Python 注释（模型看不到的地方）。

**实测**（真实问句 `给我一个一周训练计划`，真实 DashScope 云端）：
标注类措辞 **15 处 → 0 处**；唯一残留的一处「参考知识中」在**安全声明**里
（「未包含参考知识中未提及的热身细节」）——那是说明限制的正当用法，不是逐行标签。
...
README 标题此前是「🏋️ 康养知识库智能问答 RAG 系统」，而**仓库名是 `fitness-rag-coach`、
简历与 demo 里都叫「AI 健身教练」——同一个项目两个名字**。简历送出去后，
点进仓库的人会以为走错了地方（外部评审反馈第 7 条）。

统一为产品名「AI 健身教练」+ 技术定位「安全约束 RAG 问答系统」。
知识库内容确实偏康养/运动康复，该描述保留在正文中，不再作为标题。
