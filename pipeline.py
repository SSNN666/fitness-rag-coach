...
=== 回答结构（三部分，缺一不可）===
1.【基于知识的建议】— 建议要落到**具体动作名 / 禁忌 / 肌群**，且这些内容只能取自「参考知识」。
2.【知识库外推断】— 你的合理推断，必须标注「此为一般性推断，因人而异」。
3.【需要就医的情况】— 明确指出哪些情况必须咨询骨科/康复科医生。

=== 核心约束（违反即错误）===
- 正文中**禁止标注出处**：不要写「依据原文」「参考知识中」「原文步骤」这类字样
  （引用来源由系统在回答下方单独列出）。
...
# ⚠️ 第 1 部分原为「每个动作、组数、频率必须在『参考知识』中有原文依据」——
# 模型把它执行成了**逐行贴出处标签**，而它拿不到可引用的粒度（知识库是整条 CSV 行，
# 不是「第 3 步」），于是退化成空话「（依据原文步骤）」，一条回答里出现 15 次。
# 改成「内容只能取自参考知识」+ 明文禁止标注即可：**标注是 `_build_citations` 的职责**，
# 正文再标一遍是纯噪声。（这段解释只写在代码里——写进 Prompt 会把旧措辞喂回模型。）
PROMPT_PLAN = """你是健身计划制定AI。你必须仅基于「参考知识」制定训练计划。

=== 计划结构（三部分，缺一不可）===
1.【可验证部分】— 动作、组数、频率**只能取自「参考知识」**，不得新增。
2.【推断部分】— 标注「以下为一般性推断，因人而异」。
3.【安全声明】— 列出所有假设和限制条件。

=== 硬性约束 ===
- 正文中**禁止标注出处**：不要写「依据原文」「参考知识中」「原文步骤」这类字样。
  引用来源由系统在回答下方单独列出，写在正文里只是噪声。
- 禁止编造「参考知识」中不存在的动作名称。
...
class PipelineService:
    ...
    def _mode_hint(tier: str, deep_thinking: bool) -> str:
        """复杂层未开深度思考时的答后提示（缓存路径与生成路径共用，保持体验一致）。

        ⚠️ 措辞避开「快速模式」：那是 `simple/plan/injury` **分层**的词，
        而这里说的是 `deep_thinking` **开关**。用户看到计划类回答下面写「快速模式」，
        会以为系统把他降级到了简单层——实际计划层走的是主模型 + 重排 + 校验的完整链路。
        """
        if tier in ("injury", "plan") and not deep_thinking:
            return ("\n\n💡 本次未开启「深度思考」。该问题涉及伤病/复杂分析，"
                    "开启后可获得更深入的分析（耗时更长）。")
        return ""
