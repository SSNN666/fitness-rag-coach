...
| 全量测试 | **293 项通过**（原 280 + 新增 13），含熔断状态机 / 半开单探测 / 安全链路降级 |
...
> 起因：用户要求「把遗留问题一一解决」。动手前逐条核实——**6 条里 2 条与表里写的不符**。
...
**280 → 293 → 302 项**（本轮新增 9 项：指纹稳定性/增删/关系翻转、缓存键带指纹、
...
| 文档 | 用途 |
|---|---|
| [INTERVIEW_STORIES.md](INTERVIEW_STORIES.md) | 面试故事集（每个改动一条：发现过程 + 验证方式 + 讲法）。<br>原文写「8 个故事」，实际已是 28 个——**改数字会再过期，故不再写数量** |
