...
| 全量评测（重跑，含融合阈值修复） | Hit@1/3/5 = **0.80 / 0.90 / 0.94**，MRR **0.850**；RAGAS 忠实度 0.49 / 相关性 0.95 / 上下文精度 0.76 / 召回 0.64；**单伤病与复合伤病 Hit@3 均 1.00** | 2026-09-10 23:19，commit 8271c9a |
...
| **③ 原始分地板 0.40（采用）** | **0.800** | **0.905** | **0.960** | **0.857** |决定保留的依据不是指标升降，而是**覆盖率**（两次都测的交集口径）：① 有 5 条样本两次都完全无命中，② 是 4 条；**① 无命中 → ② 有命中 1 条**（「腰间盘突出患者能否进行深蹲训练？」——禁忌类问句，改前系统性漏检，改后稳定 rank 1），**① 有命中 → ② 无命中 0 条**。即：覆盖率严格变好、无任何样本退化，代价只在头部排序（Hit@1/MRR），而喂给 LLM 的 Hit@3 不变。
