...
| **融合阈值让最低权重那一路失效**（已定位并修，2026-09-10） | `retriever.py` | `FUSION_THRESHOLD` 作用在**加权分**上，而 Σw=1 且阈值(0.20)恰等于最小路由权重 → 有效门槛 = 0.20/w 随查询类型漂移：**普通 0.50 / 单伤病 0.667 / 复合 1.00**。复合伤病要求余弦 ≥ 1.00，即**向量路永远被丢弃**（实测 Milvus 返回 0.576/0.559/0.527 的高相关内容全被丢，融合结果 5/5 只有图谱）；单伤病下丢了 0.536「缓解腰部紧张的 6 个方法」。修复：向量路的过滤改用**原始余弦**地板（`VECTOR_RAW_FLOOR`，取 0.20）；BM25 是相对分（本条最高=1.0）不适用固定地板，图谱已被加权阈值覆盖，故只给向量路开。**三个口径各重复测 2~4 轮**（单次测量不足以下结论——这一点本轮栽过两次）：

| 口径 | Hit@1 | Hit@3 | Hit@5 | MRR |
|---|---|---|---|---|
| ① 改前（只用加权阈值） | 0.827 | 0.910 | 0.953 | 0.871 |
| ② 原始分地板 0.20 | 0.795 | 0.903 | 0.948 | 0.849 |
| **③ 原始分地板 0.40（采用）** | **0.800** | **0.905** | **0.960** | **0.857** |**最终口径（VECTOR_RAW_FLOOR=0.40，n=5）**：Hit@1 0.827→0.794、Hit@3 0.910→0.900、Hit@5 0.953→0.954、MRR 0.871→0.851 —— **是一次净指标让渡**，不是纯赢。
