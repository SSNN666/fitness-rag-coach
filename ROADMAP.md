...
| **融合阈值让最低权重那一路失效**（已定位并修，2026-09-10） | `retriever.py` | `FUSION_THRESHOLD` 作用在**加权分**上，而 Σw=1 且阈值(0.20)恰等于最小路由权重 → 有效门槛 = 0.20/w 随查询类型漂移：**普通 0.50 / 单伤病 0.667 / 复合 1.00**。复合伤病要求余弦 ≥ 1.00，即**向量路永远被丢弃**（实测 Milvus 返回 0.576/0.559/0.527 的高相关内容全被丢，融合结果 5/5 只有图谱）；单伤病下丢了 0.536「缓解腰部紧张的 6 个方法」。修复：向量路的过滤改用**原始余弦**地板（`VECTOR_RAW_FLOOR`，取 0.20）；BM25 是相对分（本条最高=1.0）不适用固定地板，图谱已被加权阈值覆盖，故只给向量路开。**实测指标是权衡不是纯赢**（各口径重复 2~3 次，单次测量不足以下结论）：Hit@3 / Hit@5 **无变化**（0.91 / 0.95，早期把单次 0.92/0.96 当成提升是噪声）；代价是真实的：**Hit@1 0.83 → 0.80、MRR 0.872 → 0.851**。决定保留的依据不是指标升降，而是**覆盖率**（两次都测的交集口径）：① 有 5 条样本两次都完全无命中，② 是 4 条；**① 无命中 → ② 有命中 1 条**（「腰间盘突出患者能否进行深蹲训练？」——禁忌类问句，改前系统性漏检，改后稳定 rank 1），**① 有命中 → ② 无命中 0 条**。即：覆盖率严格变好、无任何样本退化，代价只在头部排序（Hit@1/MRR），而喂给 LLM 的 Hit@3 不变。
用户可见效果显著：「腰突怎么康复」从 77 字兜底「资料不足」变成 1217 字分阶段康复方案。
⚠️ 机制补充：新纳入的低分文档并非排在末尾——`_entity_match_boost` 对命中查询实体的文档 ×1.5、不命中的 ×0.85，所以它们能跃过原本通过阈值的文档，这就是 Hit@1 下降的来源。（另测过 VECTOR_RAW_FLOOR=0.40：Hit@3 反降到 0.89、无命中回到 5，故取更松的 0.20） |
