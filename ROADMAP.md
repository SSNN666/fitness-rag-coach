...
| 问题 | 位置 | 说明 |
|---|---|---|
| ~~图谱不可用时的连接超时~~ | ~~`graph_view.py` / `retriever.py`~~ | **已于 2026-09-11 修复，见「图谱熔断与超时防护」一节。⚠️ 本行原文「`connection_timeout=8`」是错的**：8s 是 `graph_view.py`（可视化页）的参数，**热路径建 driver 时没传任何超时参数**，实际单次调用吃 34.53s。当时把可视化页的参数当成了全链路的参数 |
