...
| **融合阈值让最低权重那一路失效**（已定位并修，2026-09-10） | `retriever.py` | `FUSION_THRESHOLD` 作用在**加权分**上，而 Σw=1 且阈值(0.20)恰等于最小路由权重 → 有效门槛 = 0.20/w 随查询类型漂移：**普通 0.50 / 单伤病 0.667 / 复合 1.00**。复合伤病要求余弦 ≥ 1.00，即**向量路永远被丢弃**（实测 Milvus 返回 0.576/0.559/0.527 的高相关内容全被丢，融合结果 5/5 只有图谱）；单伤病下丢了 0.536「缓解腰部紧张的 6 个方法」。修复：向量路的过滤改用**原始余弦**地板（`VECTOR_RAW_FLOOR`，取 0.20）；BM25 是相对分（本条最高=1.0）不适用固定地板，图谱已被加权阈值覆盖，故只给向量路开。**实测指标是权衡不是纯赢**：单伤病 Hit@3 0.96→1.00、Hit@3 0.91→0.92、Hit@5 0.95→0.96、完全无命中 5→4；代价 Hit@1 0.83→0.79、MRR 0.872→0.852。用户可见效果显著：「腰突怎么康复」从 77 字兜底「资料不足」变成 1217 字分阶段康复方案。（另测过 VECTOR_RAW_FLOOR=0.40：Hit@3 反降到 0.89、无命中回到 5，故取更松的 0.20） |
